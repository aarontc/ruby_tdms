	...
	class Document
		...
		def parse_segments
			...
			until file.eof?
				...
				segment.parse_lead_in @file
				if segment.meta_data?
					segment.parse_meta_data @file
					last_meta_data_segment = segment
				end
				#
				# lead_in = @file.read 0x1C
				# metadata_pos = @file.pos
				#
				# unpacked = lead_in.unpack 'a4VVQ<Q<'
				...
				# end
				#
				next_segment_offset = segment.meta_data_offset._?(segment.raw_data_offset) + segment.length #next_seg_pos
				@file.seek next_segment_offset
			end
